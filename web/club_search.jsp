	...
	%>
 	  <div class="login">
				<form method="post" action="LoginAction.jsp">
					<input type="text" name="username" style="width:130px; height:30px;" onfocus="this.select()">
					<input type="passWord" name="password" style="width:130px; height:30px;" onfocus="this.select()">
...
<div id="wrapper">
	...
	<div id="contents">
		<form method="get" action="club_search.jsp">
			...
			<li><input type="text" id="search" name="search" size="30" value="<%=search%>" onfocus="this.select()"></li>
...
%>
	...
	<div id="img"><img src="<%=vo.getIntro_file_nm() %>" alt="동아리 포스터 이미지" width=100%; height=100%;></div>
	<div class="t1">
	    <table class="type09">
	    <thead>
	    <tr>
	        <th scope="col" colspan="4"><%=vo.getClub_nm() %>
	        <button style="float:right;">가입 신청</button>
	        </th>
	    </tr>
	    </thead>
	    <tbody>
               ...
               <table name="" id=<%=vo.getClub_nm()%> style="display: none;"
                  ...
                  <tbody>
                     ...
                     <tr>
                        ...
                        <td scope="row" style="text-align: right;">
                           <button value=<%=vo.getClub_nm()%>
                              onclick="showMore(this.value);">닫기</button>
                        </td>
                     </tr>
...
if (startPage > 1) {%>
	<a href="club_search.jsp?club_gb_cd=<%=club_gb_cd %>&search=<%=search %>&club_at_cd=<%=club_at_cd %>&pageNumber=<%=1 %>">처음</a>
<%} 
if (pageNumber > 1){%>
	<a href="club_search.jsp?club_gb_cd=<%=club_gb_cd %>&search=<%=search %>&club_at_cd=<%=club_at_cd %>&pageNumber=<%=pageNumber -1 %>">이전</a>
<% }
for(int i=startPage; i<=endPage; i++){
	if (i==pageNumber){%>
		<a href="club_search.jsp?club_gb_cd=<%=club_gb_cd %>&search=<%=search %>&club_at_cd=<%=club_at_cd %>&pageNumber=<%=i%>"><b><%=i %></b></a>	
	<%}else{%>
	<a href="club_search.jsp?club_gb_cd=<%=club_gb_cd %>&search=<%=search %>&club_at_cd=<%=club_at_cd %>&pageNumber=<%=i%>"><%=i %></a>
		<%}}%>
		
<%
if (pageNumber < totalPage){%>
	<a href="club_search.jsp?club_gb_cd=<%=club_gb_cd %>&search=<%=search %>&club_at_cd=<%=club_at_cd %>&pageNumber=<%=pageNumber +1 %>">다음</a>
<% } %>

<%
if (endPage < totalPage){%>
	<a href="club_search.jsp?club_gb_cd=<%=club_gb_cd %>&search=<%=search %>&club_at_cd=<%=club_at_cd %>&pageNumber=<%=totalPage %>">끝</a>
<% } %>
